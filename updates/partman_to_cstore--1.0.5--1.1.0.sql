...
BEGIN
    ...
    FOR _child_table, _child_table_oid, _schema, _relowner, _relacl, _datetime_string IN EXECUTE
        ...
        AND to_timestamp(SUBSTRING(ci.oid::pg_catalog.regclass::text, format(''_p(.{%s}$)'', length(p.datetime_string))), p.datetime_string) < now()-'||quote_literal(move_int)||'::interval
    ...
    IF drop_int IS NOT NULL THEN
        FOR _child_table IN
            ...
            AND to_timestamp(SUBSTRING(c.oid::pg_catalog.regclass::text, format('_p(.{%s}_cstore$)',length(_datetime_string))), _datetime_string) < now()- drop_int
